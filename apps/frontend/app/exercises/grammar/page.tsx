...
export default function GrammarExercisesPage() {
    ...
    useEffect(() => {
        async function loadThemes() {
            try {
                ...
                if (result.data) {
                    // Filter for grammar-related themes (in real app, backend would filter)
    ...
    return (
        <main id="main-content" className="grammar-exercises-page">
            <div className="container">
                ...
                <div className="page-header">
                    ...
                    <h1>Exercices de Grammaire</h1>
                    <p>Maîtrisez les règles grammaticales anglaises</p>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="themes-grid">
                        {themes.slice(0, 8).map((theme, index) => (
                            <Link
                                key={theme.id}
                                href={`/exercises/grammar/${theme.id}?level=${selectedLevel}`}
                                className="theme-card"
                            >
                                <div className="theme-icon">{theme.icon || '📚'}</div>
                                <h3>{theme.name}</h3>
                                <div className="theme-meta">
                                    <span className="exercise-count">12 exercices</span>
                                    <Play size={16} />
                                </div>
                            </Link>
                        ))}
                        ...
                        <div className="theme-card locked">
                            ...
                            <div className="theme-meta">
                                <span className="exercise-count">25 exercices</span>
                            </div>
                        </div>
                    </div>
                )}
            ...
            <style jsx>{`
                .grammar-exercises-page {
                    min-height: 100vh;
                    padding: 3rem 0;
                    background: linear-gradient(135deg, #dbeafe 0%, var(--background) 50%);
                }
